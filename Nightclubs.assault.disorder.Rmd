...
#add more controls: proximity to university, to police station, below poverty line, distance from homeless shelter
```
...
fit_1 <- lm(arrests_count ~ liquor_count + tot_pop + med_male_age +
              ...
              tot_white + tot_black + tot_hisp + popdens, data=main_sf)
...
In this simple OLS specfication,  only liquor location count and total black population are significant.

[mention racial bias again]
...
fit_NW<- lm(arrests_count ~ liquor_count + tot_pop + med_male_age +
              tot_males_20_24 + tot_males_25_29 + tot_males_not_labforce +
              tot_white + tot_black + tot_hisp + popdens, data=main_NW_sf)
...
fit_NE<- lm(arrests_count ~ liquor_count + tot_pop + med_male_age +
              tot_males_20_24 + tot_males_25_29 + tot_males_not_labforce +
              tot_white + tot_black + tot_hisp + popdens, data=main_NE_sf)
...
fit_SW<- lm(arrests_count ~ liquor_count + tot_pop + med_male_age +
              tot_males_20_24 + tot_males_25_29 + tot_males_not_labforce +
              tot_white + tot_black + tot_hisp + popdens, data=main_SW_sf)
...
fit_SE<- lm(arrests_count ~ liquor_count + tot_pop + med_male_age +
              tot_males_20_24 + tot_males_25_29 + tot_males_not_labforce +
              tot_white + tot_black + tot_hisp + popdens, data=main_SE_sf)
...
lm.morantest(fit_SW, rwm_SW, alternative="two.sided") #not sure what happened here, sample size too small?

lm.morantest(fit_SE, rwm_SE, alternative="two.sided") 

```

P-value is not significant for NW, NE, and SE. No need to run multiple spatially lagged models for each quadrant. Can proceed with one spatially lagged model for all of DC. 


```{r spatially lagged model}
#Then we create a list of neighbours using the Queen criteria
fit_lagged <- errorsarlm(arrests_count ~ liquor_count + tot_pop + med_male_age +
              tot_males_20_24 + tot_males_25_29 + tot_males_not_labforce +
              tot_white + tot_black + tot_hisp + popdens, data=main_sf, rwm)

summary(fit_lagged)

```

Here, only liquor licensed locations is significant, whereas with the simple OLS model both liquor licensed locations and total black population were signifcant. 
