...
```{r sub region analysis}

quads<-st_read("./datasets/DC_Quadrants-shp/eb24d650-e20e-43b7-87de-6a603c970342202043-1-v0zhf7.oyql.shp")

within_ward <- st_join(tracts_shp, quads, join = st_intersects)
within_ward<-subset(within_ward, select=c("QUADRANT", "TRACT"))
within_ward$geometry<-NULL

main<-left_join(main, within_ward, by="TRACT")

ggplot(main, aes(x = res_fit1, colour = as.factor(QUADRANT))) + 
  geom_density() 
...
#fit_2_lag <- lagsarlm(arrests_count ~ liquor_count + tot_pop + med_male_age +
              #tot_males_20_24 + tot_males_25_29 + tot_males_not_labforce +
              #tot_white + tot_black + tot_hisp + popdens, data=main, w)
