...
nbh<- st_read("./datasets/Neighborhood_Clusters/Neighborhood_Clusters.shp")
...
income_shp <-st_read("./datasets/ACS_2018_Median_Household_Income_Variables_Tract-shp/c9ca5f40-0f43-4de6-a527-28440f3bdf132020330-1-9idood.na0x.shp")
...
#coefficients
head(fit_1$coefficients)
#p-values
summary(fit_1)$coefficients[,4]  
#r-squared
...
p-value of Moran's I is not significant, therefore we cannot reject the null that arrests for assault and disorderly conduct are randomly distributed throughout Census tracts in DC.

If Moran's I were significant, indicating the presence of spatial autocorrelation, we would need to do a spatial regression model.  
...
#unemployment rate total unmarried males not in labor force B12006_007E
...
acs2<- gather(acs, name, TRACT) 
acs2 <-spread(acs, variable, estimate)


PC<- gather(PC, Year, Value, "2015":"2020") 
PC <-spread(PC,Series,Value)
