...
fit_1 <- lm(arrests_count ~liquor_count +tot_pop +
             med_hh_income + hhsize_rental + 
              white + black +  males_25_29 + poverty +
              not_labforce + popdens, data=main_sf)
...
The p-value of Moran's I is on the edge of significance, therefore we may want to reject the null that arrests for assault and disorderly conduct are randomly distributed throughout Census tracts in DC.

A significant Moran's I suggests the presence of spatial autocorrelation, so we should conduct a spatial regression model.  
...
fit_NW<- lm(arrests_count ~liquor_count +tot_pop +
             med_hh_income + hhsize_rental + 
              white + black +  males_25_29 + poverty +
              not_labforce + popdens, data=main_NW_sf)
...
fit_NE<- lm(arrests_count ~liquor_count +tot_pop +
             med_hh_income + hhsize_rental + 
              white + black +  males_25_29 + poverty +
              not_labforce + popdens, data=main_NE_sf)
...
fit_SW<- lm(arrests_count ~liquor_count +tot_pop +
             med_hh_income + hhsize_rental + 
              white + black +  males_25_29 + poverty +
              not_labforce + popdens, data=main_SW_sf)
...
fit_SE<- lm(arrests_count ~liquor_count +tot_pop +
             med_hh_income + hhsize_rental + 
              white + black +  males_25_29 + poverty +
              not_labforce + popdens, data=main_SE_sf)
...
#Then we create a list of neighbours using the Queen criteria
fit_lagged <- errorsarlm(arrests_count ~liquor_count +tot_pop +
             med_hh_income + hhsize_rental + 
              white + black +  males_25_29 + poverty +
              not_labforce + popdens, data=main_sf, rwm)

summary(fit_lagged)

```

In the original OLS model, the variables significant at the 5% level were: 

  1. Number of on-premise liquor licensed locations
  2. Total population
  3. Median household income
  4. Median household size of rental units
  5. Percentage of total males that are 25-29 
  6. Percentage of total population not in the labor force
  

In the spatially lagged model, variables significant at the 5% level are: 
