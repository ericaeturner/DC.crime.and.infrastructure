...
In our final project, we will be looking at the relationship between different types of infrastructure in DC and different types of crime using geospatial analysis. In this first piece of analysis, we look at the relationship between the locations of 
 ...
 1. Bars 
 2. Simple assaults and disorderly conduct occurring late at night
...
Here, we narrowed down arrests to simple assaults and disorderly conduct, occurring in 2017, between the hours of 12am and 4am, and those not missing data for longitude or latitude. 
...
#tracts_shp<-tracts_shp[-c(12),] #may want to delete tract 2.02 later, the one missing income data
...
income_shp$med_hh_income[is.na(income_shp1$med_hh_income)] <- 174999 
#delete the other tract for which there is no data available 
...
IVs <- left_join(pop_shp, income_shp, by="TRACT")

IVs<-IVs %>% st_sf(sf_column_name = 'geometry.x')
IVs$geometry<-IVs$geometry.x
...
(IVs$arrest_count <- lengths(st_intersects(tracts_shp, arrests_sf)))
(IVs$liquor_count <- lengths(st_intersects(tracts_shp, liquor_sf)))
