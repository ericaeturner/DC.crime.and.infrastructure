...
This can be explored further and more formally through Ripley's K method. The cross-K function used here, is based on the average number of offenses within a certain distance of a liquor license location. It compares the observed data to a distribution that would be expected if the points were randomly distributed (rather than clustered in relation to each other). It essentially answers the question - could this clustering have occured simply by random chance? We use the inhomogenous version of K-cross as it is evident from the density plot that the distribution of points is not homogenous/uniform. 

The envelope function help understand the significance of any pattern detected. It uses a Monte Carlo simulation randomly assigning points as offense or liquor license points. It runs such a simulation 99 times and if the curve representing the observed points falls within the confidence interval of the simulation, it implies that the pattern of clustering doesn't change even when the the categories are randomly assigned. That shows no significant relation between the location of liquor stores and offenses. 